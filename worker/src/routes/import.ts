...
importRoutes.post('/:id/import/deezer', async (c) => {
  ...
  const playlistId = await resolveDeezerPlaylistId(body.playlist_url?.trim() ?? '')
  if (!playlistId) return c.json({ error: 'invalid Deezer playlist URL' }, 400)
  ...
  let urls: string[]
  try {
    const result = await fetchDeezerPlaylistTracks(playlistId)
    urls = result.urls
  } catch (err) {
    return c.json({ error: err instanceof Error ? err.message : 'Deezer error' }, 502)
  }
